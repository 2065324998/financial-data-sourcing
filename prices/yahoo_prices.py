...
def download_prices(
    ...
    print(f"Downloading prices from Yahoo Finance ({start} to {end})...")
    data = yf.download(
        ticker_str,
    ...
    # yfinance returns multi-level columns when >1 ticker: (field, ticker)
    # single ticker returns flat columns
    result: Dict[str, Dict[date, float]] = {t: {} for t in tickers}
    ...
    if len(tickers) == 1:
        ...
        for ts, val in series.items():
            ...
            result[ticker][d] = float(val)
    else:
        # Multi-level: data["Close"][ticker]
        try:
            close_data = data["Close"]
        except KeyError:
            print("  WARNING: no Close column in multi-ticker data")
            return result

        for ticker in tickers:
            # yfinance may normalise ticker names (BRK-B -> BRK-B)
            col = ticker
            if col not in close_data.columns:
                # try alternate form: BRK-B <-> BRK.B
                alt = ticker.replace("-", ".") if "-" in ticker else ticker.replace(".", "-")
                if alt in close_data.columns:
                    col = alt
                else:
...
def run(
    ...
    print()
    price_data = download_prices(tickers, start, end)
...
Examples:
  ...
  python3 prices/yahoo_prices.py --tickers AAPL MSFT --dry-run
    ...
    run(tickers=args.tickers, start=start, end=end, dry_run=args.dry_run)
